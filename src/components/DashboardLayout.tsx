...
export default function DashboardLayout({ children, title, role = "admin" }: DashboardLayoutProps) {
  ...
  // Auth guard: redirect to correct login page if not logged in
  useEffect(() => {
    const checkAuth = async () => {
      ...
      const { data: profile } = await supabase
        .from("profiles")
        .select("full_name, institute_code, role")
        .eq("user_id", user.id)
        .single();
    ...
    <div className="flex flex-col h-full">
      ...
      <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
        {menuItems.map((item) => {
          ...
          // Admin-only pending badge: only show on approvals and batch-applications for admins
          const showBadge = isAdmin && pendingCount > 0 && (item.path === "/admin/approvals" || item.path === "/admin/batch-applications");
          return (
            <Link
